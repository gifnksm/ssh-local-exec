...
async fn communicate(
    ...
    let _stdin_thread = {
        ...
        thread::Builder::new()
            .name("stdin".into())
            .spawn(|| read("stdin", io::stdin(), stdin_bytes_tx, stdin_res_rx).in_current_span())
    ...
    let stdout_thread = thread::Builder::new()
        .name("stdout".into())
        .spawn(|| write("stdout", io::stdout(), stdout_res_tx, stdout_bytes_rx).in_current_span())
        .wrap_err("failed to spawn thread")?;
    ...
    let stderr_thread = thread::Builder::new()
        .name("stderr".into())
        .spawn(|| write("stderr", io::stderr(), stderr_res_tx, stderr_bytes_rx).in_current_span())
        .wrap_err("failed to spawn thread")?;
